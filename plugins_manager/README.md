...
**אומת:** 43 בדיקות יחידה (`dart test`) עוברות — קריאת `manifest.json`
מ-`.otzplugin` אמיתי שנבנה ב-`archive` (כולל BOM, ZIP פגום, manifest
חסר), סריקת תוספים מותקנים מול עץ תיקיות זמני, round-trip של
`catalog.json` (כולל קטלוג פגום חלקית, קטלוג ישן בלי קטגוריות, ותאימות
`isPinned`), פירוק `Content-Disposition` (כולל שמות עבריים), השוואת
גרסאות והכרעת מצב ההתקנה, ודחיית קובץ חסר / סיומת שגויה בהתקנה הישירה.
בנוסף **סנכרון מלא מול אתר מדומה** (`MockClient`): תוספים + קטגוריות +
טקסטים, סדר ידני מדף הקטגוריה, דילוג על מזהה שאינו בקטלוג, ושמירת
המבנה הקודם כש-`/api/plugins/store-home` מחזיר 404. `dart analyze` נקי.
