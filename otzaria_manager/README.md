...
- `CFBundleShortVersionString = 0.9.96` — כלומר תג ה-release **בלי**
  ...
  **אבל ה-build לא נזרק לפח:** אוצריא מתקנת לפעמים באג ומפרסמת מחדש תחת
  אותו מספר גרסה, כשכל השינוי הוא ה-`+736` → `+741`. `sameBuild` מכריע
  בו — ורק כששני הצדדים נושאים אחד, כך שצד שנקרא מתוך התקנה קיימת (ושם
  לעולם אין build) ממשיך להיחשב זהה.
...
- **`/DIR=` נמסר רק כשמעדכנים התקנה קיימת.** בהתקנה חדשה בווינדוס הוא
  נעדר, והמתקין מתקין ל-`DefaultDirName` שלו. הסיבה: ברירת המחדל שהקוד
  הזה "ידע" (`{autopf}\Otzaria`, מפי המפתחים ב-2026-08-07) אינה מה שכתוב
  ב-iss (`DefaultDirName=C:\אוצריא`) — ניחוש שמתיישן גרוע מלא לנחש, ואת
  התשובה האמיתית ממילא מקבלים מ-`InstallLocation` שברג'יסטרי אחרי ההתקנה.
  ⚠️ `resolveDefaultInstallDir()` נשארת בשימוש **ב-macOS בלבד**, שבו אין
  מתקין שבוחר; התיעוד שבתוכה על "ברירת המחדל האמיתית" הוא זה שהתיישן.
- **`/MERGETASKS=desktopicon` — קיצור הדרך בשולחן העבודה.** ב-`installer/otzaria.iss`
  וב-`installer/otzaria_full.iss` שבריפו **`Otzaria/otzaria`** (הריפו שממנו
  מורידים כאן — יש גם `Sivan22/otzaria` עם קבצים דומים אך לא זהים, אל תאמת
  מולו) המשימה `desktopicon` מוגדרת `Flags: unchecked`, ולכן התקנה שקטה
...
- **כל התקנה שהמשתמש יזם רצה עם האשף, לא בשקט** — גם החבילה המלאה וגם
  ...
  (`DisableDirPage=auto` מסתיר אותה רק כשיש התקנה קודמת עם אותו AppId)
  וקיצור הדרך — ואלה בדיוק ההחלטות שרק המשתמש יכול לקבל: התקנה לכונן נייד,
  או לתיקייה של משתמש אחד. במסלול הזה **אין שום דגל** מלבד `/LOG=`: לדרוס
  בחירה בדגל היה מחזיר את הבעיה. אחרי שהאשף נסגר מזהים לאן הותקן
  (רג'יסטרי + מיקומים מוכרים).
- **כשיש התקנה קיימת, הגרסה החדשה נכנסת בדיוק לתיקייה שלה.** גם במסלול
  האשף נמסר `/DIR=<התיקייה הקיימת>` — ב-Inno הדגל קובע את *ברירת המחדל* של
  עמוד היעד, ולכן הוא אינו סותר את האשף: העמוד ממילא מוסתר בעדכון
  (`DisableDirPage=auto`), וכשהוא מוצג המשתמש עוד יכול לשנות. להסתמך על
  `UsePreviousAppDir` של Inno לבדו לא מספיק — הוא עובד רק כשהוא מוצא את
  רשומת ההתקנה שלו עצמו, ולא כשהמשתמש הצביע ידנית על התקנה שאין לה רשומה
  (התקנה ניידת, או כזו שנרשמה תחת משתמש אחר). התיקייה שנמסרה גם **קודמת
  לזיהוי** בקביעת מה הותקן, כדי שהתקנה אחרת שנשארה במחשב לא תיחשב לתוצאה;
  אם אין בה קובץ הרצה — סימן שהמשתמש שינה יעד באשף — נופלים לזיהוי הרגיל.
- **אחרי אשף שהצליח מריצים בדיקה מחדש** (`OtzariaModuleController.install`).
  Inno שמתרומם להרשאות מנהל מחזיר 0 מהתהליך שהרצנו לפני שההתקנה נגמרה, ולכן
  "הצליח" אינו עדות לגרסה שעל הדיסק — הגרסה נקראת מקובץ ההרצה עצמו.
- **ההתקנה האוטומטית נשארת שקטה.** מי שהדליק "התקן אוטומטית" ביקש שזה
...
- **ביטול באשף אינו שגיאה**: `OtzariaInstallCancelled` (קודי יציאה 2/5 של
  ...
  והתהליך שהרצנו חוזר מיד. שניהם מוצגים כהודעה רגילה, לא כתקלה.
- ⚠️ **קוד יציאה 1 אינו כשל כאן** (issue #26). ב-Inno הוא אומר
  "`InitializeSetup` החזיר `False`", ו-`otzaria.iss`/`otzaria_full.iss`
  משתמשים בו בדיוק כדי לפרוש מהתהליך הנוכחי **אחרי** ששיגרו את המתקין
  מחדש: מורם ב-`ShellExecute('runas')` כשההתקנה הקודמת יושבת בנתיב שדורש
  מנהל (גם התקנה ישנה שהלאנצ'ר עצמו לא זיהה — ה-iss מוצא אותה ברג'יסטרי),
  ובשקט `/SILENT` בשדרוג מגרסה 0.9.88 ומעלה. ההתקנה נמשכת בתהליך השני,
  שאותו איננו מחזיקים — ולכן `wizardOutcomeFor(1)` הוא
  `OtzariaWizardOutcome.relaunched`, שממשיך לזיהוי בדיוק כמו `finished`,
  ובסופו `OtzariaWizardStillOpen` ("לחץ בדיקה מחדש") ולא שגיאה. המחיר:
  כשל אתחול אמיתי (למשל ארכיטקטורה לא נתמכת) יקבל את אותה הודעה — אבל שם
  המתקין עצמו כבר הקפיץ `MsgBox` שמסביר מה קרה. במסלול השקט אין את
  הבעיה: `WizardSilent` יוצא מ-`InitializeSetup` לפני כל שיגור-מחדש.
- ⚠️ **ב-`otzaria_full.iss` יש `[Code]`**: `InitializeSetup` מקפיץ `MsgBox`
  שמזהיר שנתונים ב-`אוצריא\` וב-`links\` יימחקו. במסלול האשף המשתמש רואה
  את השאלה; במסלול השקט `/SUPPRESSMSGBOXES` עונה עליה בברירת המחדל, כלומר
  מאשר בשמו. הספרייה עצמה מחולצת ב-`ssPostInstall` מ-`otzaria_latest.zip`
  דרך `Expand-Archive` אל `{app}` — כלומר לאן שהמשתמש בחר.
...
- **מבנה בתוך ה-installer**: פעם הקובץ המותקן ישב ב-`app/otzaria.exe`
  (אומת עם `innoextract` על installer אמיתי), אבל ב-iss הנוכחי הוא יושב
  ישירות בשורש תיקיית ההתקנה. לכן `OtzariaAppLocator` **סורק** ולא מניח
  נתיב — אין להחזיר אותו להנחה.
