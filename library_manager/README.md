...
- **הכתיבה היחידה שלנו להגדרות של אוצריא: מיקום הספרייה, אחרי התקנה
  טרייה** (ספטמבר 2026, [פוסט 39342](https://otzaria.org/forum/post/39342)).
  ⚠️ **אוצריא אינה סורקת את ברירת המחדל שלה עצמה**:
  `DatabaseConstants.getDatabasePath` נופל ל-`'.'` כש-`key-library-path`
  ריק, ו-`getDefaultLibraryPath` משמש רק בהורדה מתוכה. ספרייה שהתקנו
  ב-`%APPDATA%\otzaria\books` הייתה לכן בלתי-נראית לה, והמשתמש נשאר עם מסך
  "ספרייה ריקה". [`OtzariaSettingsWriter`](lib/src/services/otzaria_settings_writer.dart)
  כותב אחרי התקנה טרייה את `key-library-path` (ו-`key-library-folder-name`
  ריק, בדיוק כמו `EmptyLibraryBloc` שם). הגבולות: **רק** כשההגדרה עדיין
  ריקה — בחירה של המשתמש לא נדרסת לעולם — ורק כשאוצריא סגורה, ככל עדכון
  ספרייה. כשל מחזיר `false`, וה-UI אומר למשתמש להצביע על התיקייה בעצמו.
  ⚠️ חמישה דברים הופכים את הפתיחה-במקום לבטוחה, וכל אחד מהם היה באג
  בגרסה הראשונה של הקוד: `crashRecovery: false` (ברירת המחדל של Hive
  **מקצצת** קופסה שאינה נפרסת, כלומר מוחקת את ההגדרות של המשתמש לפני
  שכתבנו מילה); בדיקת `OtzariaProcessGuard` מיד לפני הכתיבה (הפתיחה
  לוקחת נעילה בתיקייה של אוצריא); אימות `box.path` (Hive מאתר קופסה
  פתוחה **לפי שם בלבד** ומתעלם מה-`path`); `putAll` אחד במקום שני `put`
  (קריסה ביניהם משאירה נתיב חדש עם שם תיקייה ישן); ו-`otzariaSettingsRoot`
  ולא `otzariaDataRoots.first` (בלי `%APPDATA%` הראשון הוא `%ProgramData%`,
  שאוצריא לא קוראת ממנו הגדרות לעולם).
- **התקנת מנהל מחפשת קודם ב-`%ProgramData%`.** `AppPaths.getDefaultLibraryPath`
  מחזיר שם את `%ProgramData%\otzaria\books`, ולכן `isSystemInstall`
  (סימון `system_install.marker` ליד ה-exe, או exe תחת `Program Files`;
  התקנה ניידת לעולם לא) הופך את סדר שתי ברירות המחדל. ⚠️ קודם לכן `%APPDATA%`
  ניצח תמיד — כלומר בהתקנה מערכתית התקנו לתיקייה שאוצריא לא מחפשת בה.
...
- **⚠️ התקנה טרייה נרשמת ב-`customDbPath` רק כשהאיתור לא ימצא אותה לבדו.**
  `_finishDbUpdate` רשם כל התקנה טרייה, כלומר הפך **ניחוש** ל**בחירה של
  המשתמש** — אותו שדה שהבורר הידני כותב, ושנבדק ב-`resolveDbPath` *לפני*
  ההגדרות של אוצריא. מסד שהמשתמש העביר למקום אחר מזוהה אך ורק דרך
  `app_preferences.hive`, ו-`OtzariaSettingsReader` מחזיר `null` על כל כשל —
  קופסה שלא נפתחה, סחיפת פורמט, או אוצריא ניידת שלא זוהתה (בלי `launchPath`
  שורש הנתונים שלידה כלל לא נסרק). כלומר כשל קריאה אחד הספיק כדי לכתוב מסד
  מלא ל-`%APPDATA%` בזמן שהספרייה האמיתית יושבת במקום אחר, ואז **לנעול** את
  הלאנצ'ר על הנתיב השגוי — גם אחרי שהקריאה חזרה לעבוד. התנאי
  `!isKnownToOtzaria(dbPath)` משאיר את הרישום רק ליעד שהאיתור לא סורק
  בעצמו (הנפילה ל-`<dataDir>/library`), ומחזיר ללאנצ'ר את היכולת לתקן את
  עצמו בהרצה הבאה. `library_db_locator_test.dart` מצמיד את התכונה שזה נשען
  עליה: יעד שנגזר מ-`resolveInstallDbPath` הוא תמיד `isKnownToOtzaria`.
...
- **שינוי סכמה = מסד מלא, כמו באוצריא** (ספטמבר 2026) — כשהמאגר מפרסם
  גרסה בסכמה שאין לה סדר hash (v26 יצאה בסכמה 4; ראו
  `kHashTableOrderBySchemaVersion`), אין מסלול patches אליה. אוצריא
  המקוונת מורידה במקרה כזה את המסד המלא מחדש, וכך גם כאן: הקשתות האלה
  מסוננות ב-`LibraryUpdateDiscovery`, המראה אינה נושאת את קובציהן, ובחירת
  המסד המלא (`_chooseFullDbCarrier`) כבר אינה סופרת אותן כ"מסלול" — ולכן
  מסד מלא ישן שכבר על הכונן מפנה את מקומו לחדש. שתי הגנות נוספות מאותה
  תקלה: תוכנית שאינה עוברת את הגרסה המקומית חוזרת כ-`blocked` מנומק במקום
  להחליף מסד עדכני בישן, ו-`applyDelta` מדווח את הגרסה שכל צעד הגיע אליה
  כדי ששרשרת שנקטעה תירשם נכון.
...
- **עומק שרשרת הדלתא** — המראה שומרת את עשרת ה-releases האחרונים
  ...
  ה-patches בלבד; את ה-DB המלא (~1.5GB) מורידים **פעם אחת**, והוא היחיד
  שהמסלול המלא באופליין בוחר.
- **מסד מלא שכבר במראה אינו מוחלף** — כל release ב-SeforimLibrary נושא
  `seforim.db.zst` משלו, ולכן `LibraryMirrorExporter._chooseFullDbCarrier`
  מעדיף את זה שכבר על הכונן כל עוד יש ממנו מסלול patches ל-latest. בלי זה כל
  עדכון היה מוריד ~1.1GB בשביל תוצאה שקובצי עדכון מגיעים אליה בדיוק באותה
  מידה. המחיר: הורדה מלאה נוחתת על גרסה ישנה מ-latest, ולכן `plan()` מצרף
  לה `followUpDelta` ו-`applyUpdate()` מריץ את שני הצעדים ברצף אחד —
  התקנה על מחשב ריק עדיין מסתיימת בגרסה האחרונה.
- **מצב "עדכון אישי"** — תוספת שאין באוצריא: `personalUpdateMode` מצמצם את
  ...
  אזהרה. `isRegisteredForPersonalUpdate()` עונה אם **המחשב הזה** בין הנרשמים —
  מה שמבדיל חסימה אמיתית ממראה שנבנתה למחשב אחר. ראו AGENTS.md §1.
- **הורדה מקבילה** — כאן יש תוספת שאין באוצריא: נכסי הספרייה יורדים
  בו-זמנית (עד ארבעה, הגדול ראשון), ו-`downloadToMirror` מריץ את הקבצים
  הנלווים **לצד** הייצוא ולא אחריו — הם ~509MB שהמתינו בתור להורדת המסד
  (~1.55GB) בלי סיבה. שניהם חולקים `DownloadScheduler` אחד, ולכן התקרה היא
  ארבעה חיבורים לשניהם יחד. פיצול קובץ בודד לכמה חיבורים אינו אפשרי מול
  GitHub — ראו AGENTS.md §5.
...
#### שלמות התוכן של התלמוד, ולא רק הסימון שמעליו

**סימון גרסה אינו התוכן שהוא מייצג.** הפרדיקט שאל רק את
`תלמוד בבלי/.version`, ולכן תיקייה שנשאר בה קובץ אחד מתוך כל הש"ס נקראה
"מעודכנת" לנצח ([issue #33](https://github.com/Otzaria/Otzaria_Offline_update/issues/33)).
הסימון הזה נכתב לרוב בידי אוצריא עצמה אחרי הורדה מהרשת, ומה שקרה לקבצים
אחר כך — העתקה שנקטעה, אנטי-וירוס, דיסק — אינו מופיע בו.
`_talmudContentsComplete` שוקל לכן גם את התיקייה, בשני מדרגים:

1. **מפרט מדויק** — בכל התקנה שלנו נכתב לצד הסימון `.version.contents`:
   מספר הקבצים, סך הבייטים, ו**הסימון שעבורו נכתב**. תיקייה שירדה מתחתיו
   חוזרת להצעה.
2. **רצפה גסה**, כשאין מפרט (אוצריא התקינה, או לאנצ'ר ישן) — ה-PDF-ים כבר
   דחוסים, ולכן חילוץ שלם שוקל בערך כמו הארכיון, ורבע ממנו הוא חוסר שאי
   אפשר לטעות בו. דחיסה אמיתית רק מרחיבה את המרווח.

**הכיוון המסוכן הוא "לא שלם".** הוא מציע חילוץ של ~450MB, והצעה כזו שחוזרת
בכל פתיחה גרועה מהבאג שהיא מתקנת. שלוש ההגנות שנובעות מכך: קובץ ש**נוסף**
לתיקייה אינו חוסר (`>=`, לא `==`), מפרט שנכתב עבור סימון אחר מתעלמים ממנו
במקום להשוות אליו תוכן של גרסה אחרת, וארכיון קטן מ-`talmudFloorMinArchiveBytes`
אינו נבדק ברצפה כלל — שם תקורת ה-tar (512 בייט לכל קובץ) יכולה לבדה לעבור
את המרווח. כתיבת המפרט עצמה היא best-effort: כשל בה אינו מבטל חילוץ שהצליח,
כי בהיעדר מפרט הבדיקה נופלת לרצפה שהתקנה שלמה עוברת ממילא.
...
המחיר של פתרון מבוסס-קישור לעומת קריאת הקובץ. לסגור את הפער מהצד שלנו אי
אפשר: `booksDone` ואינדקס ה-tantivy הם מבנים פנימיים של אוצריא, וכתיבה
אליהם מבחוץ עוקפת את המנגנון שלה — בדיוק כמו חילוץ ידני של `.otzplugin`.
...
>
> **וממראה מקומית אין בכלל שלב הורדה.** הנכס כבר קובץ על הכונן, ולכן
> `applyFullDownload` מחלץ **ממנו** ישירות (`PatchDownloader.isRemoteUrl`
> הוא ההבחנה) במקום להעתיקו קודם ל-`<db>.download.zst`: חסכון של כתיבה
> וקריאה של ~1.5GB, ושיא התפוסה יורד מ*ישן + דחוס + חדש* ל*ישן + חדש*.
> ה-sha256 שההעתקה נהגה לאמת מחושב עכשיו בתוך החילוץ, על אותם בתים
> (`ZstdFileDecompressor.onSourceDigest`) — מעבר אחד, לא שניים.
> **המראה עצמה לעולם אינה נמחקת**: כל מסלולי הניקוי נוגעים אך ורק בקובץ
> שהתוכנה יצרה בעצמה (`deleteStaged`), ומחיקה של המקור הייתה מרוקנת את
> הכונן שממנו אמורים להתקין גם במחשב הבא.
>
...
6. **מעבר הסכמה (סכמה 4, ספטמבר 2026) נבדק ביחידות בלבד.** הסינון, החסימה
   המנומקת, ובחירת המסד המלא החדש מכוסים בבדיקות מעל מראה מדומה. מה שלא
   הורץ: ייצוא מראה אמיתי מ-SeforimLibrary אחרי מעבר הסכמה (כלומר שהמסד
   המלא של v26 אכן קיים ב-release ויורד), והחלפה בפועל של מסד חי במסד
   בסכמה 4 — כולל השאלה שמחוץ לתחום שלנו, האם אוצריא שמותקנת אצל המשתמש
   יודעת לקרוא סכמה 4.
