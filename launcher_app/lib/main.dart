  ...
  runZonedGuarded(
    () async {
      ...
      try {
        ...
        debugPrint('$e');
        // **גם מסך העצירה משאיר עקבה.** זה היה המסלול היחיד שנגמר בלי שורת
        // לוג אחת בשום מקום — ולכן משתמש שנתקע בו נראה בדיוק כמו משתמש
        // שהתוכנה שלו לא כותבת ללוג בכלל. ה-temp הוא מה שנשאר כשלתיקיית
        // הנתונים אין הרשאת כתיבה.
        try {
          await AppLogger.init(
            Directory.systemTemp.path,
            version: launcherVersion,
          );
          AppLogger.instance.error('תיקיית הנתונים אינה זמינה לכתיבה: $e');
        } catch (_) {}
      ...
      // הכונן מוגן מכתיבה: המצב והלוג יושבים מעכשיו במחשב הזה, והתיקייה
      // שעל הכונן קופאת. בלי השורה הזאת אין בלוג שום רמז לאיזה משני
      // המיקומים הוא שייך.
      if (paths.readOnly) {
        AppLogger.instance.warn(
          'הכונן מוגן מכתיבה — מצב והגדרות נשמרים במחשב הזה: '
          'dataDir=${paths.dataDir} stateDir=${paths.stateDir}',
        );
      }
