...
**`syncFullPackage`** ("חבילת התקנה מלאה של אוצריא", כבוי בברירת מחדל) מוסיף
להורדה את המתקין שכולל את הספרייה בתוכו — **הגרסה היציבה האחרונה בלבד**,
~2GB. הוא מיועד למחשב שאין בו אוצריא: התקנה אחת מביאה תוכנה וספרייה. מי
שאינו מדליק אותו אינו רואה שום שינוי בתוכנה — הכרטיס במסך התוכנה מוצג רק
כשקובץ ההתקנה באמת על הכונן, וההמלצה להתקין ממנו מופיעה רק כשגם לא זוהתה
שום התקנה של אוצריא. כיבוי ההגדרה מוחק את החבילה מהכונן בהורדה הבאה.
...
והחבילה המלאה אינה שם כלל (היא מוצעת רק כשאין אוצריא במחשב). הסיבה היא
