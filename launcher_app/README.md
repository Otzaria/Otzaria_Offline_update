...
### חלוקת התוכנות הנוספות: ניהול במסך ייעודי, שימוש במסך החנות
...
הוספה, עריכה, הסרה, קטגוריות וסדר התצוגה — יושב ב-
`custom_apps/custom_apps_manage_dialog.dart`. `CustomAppsScreen` נשאר למה
שעושים עם התוכנות עצמן: הורדה לכונן, בדיקה ברשת, התקנה, בחירת מיקום
ידנית והפעלה.
...
- **`openAddCustomApp`/`openEditCustomApp` יושבות במסך הניהול** — הן
  היחידות שפותחות את `CustomAppFormDialog`, ומסך החנות אינו מייבא אותו
  כלל. משם נפתח גם `showCustomAppCategoriesDialog`, מאותה סיבה בדיוק.
- **אין במסך החנות שום דרך לנהל, גם לא הפניה.** מי שרוצה להוסיף, לערוך
  או להסיר הולך להגדרות — בדיוק כמו כל הגדרה אחרת בלאנצ'ר.
- **סדר התצוגה נקבע בחיצי מעלה/מטה**, כמו צילומי המסך בטופס, ונשמר
...
במצב הזה הספרייה **תמיד** נכללת בהורדה: `downloadsLibrary` (`syncLibrary ||
personalUpdateMode`) הוא מה שנקרא בכל מקום, ולא `syncLibrary` עצמו. הבחירה
השמורה אינה נדרסת, כך שכיבוי המצב מחזיר אותה. בהגדרות הסגמנט "ספרייה" מסומן
ומושבת, ו"הכל" נוגע אז רק בתוכנה ובתוספים.
